...
public class AlfrescoAccountManager extends Manager
{
    ...
    public static List<AlfrescoAccount> retrieveAccounts(Context context)
    {
        ...
        try
        {
            ...
            android.accounts.Account[] accountMs = mAccountManager.getAccountsByType(AlfrescoAccount.ACCOUNT_TYPE);
    ...
    public AlfrescoAccount retrieveAccount(long id)
    {
        ...
        android.accounts.Account[] accounts = mAccountManager.getAccountsByType(AlfrescoAccount.ACCOUNT_TYPE);
    ...
    public List<android.accounts.Account> getAndroidAccounts()
    {
        List<android.accounts.Account> accounts = null;
        try
        {
            AccountManager mAccountManager = AccountManager.get(appContext);
            android.accounts.Account[] accountMs = mAccountManager.getAccountsByType(AlfrescoAccount.ACCOUNT_TYPE);
            accounts = new ArrayList<android.accounts.Account>(accountMs.length);
            for (android.accounts.Account account : accountMs)
            {
    ...
    public android.accounts.Account getAndroidAccount(long id)
    {
        AccountManager mAccountManager = AccountManager.get(appContext);
        android.accounts.Account[] accounts = mAccountManager.getAccountsByType(AlfrescoAccount.ACCOUNT_TYPE);
        if (accounts.length == 0) { return null; }
    ...
    public String createUniqueAccountName(String defaultName)
    {
        ...
        AccountManager mAccountManager = AccountManager.get(appContext);
        Map<String, android.accounts.Account> accountIndex = null;
        android.accounts.Account[] accounts = mAccountManager.getAccountsByType(AlfrescoAccount.ACCOUNT_TYPE);
        if (accounts.length > 0)
        {
            accountIndex = new HashMap<String, android.accounts.Account>(accounts.length);
            for (android.accounts.Account accountAvailable : accounts)
            {
    ...
    {
        long accountIndex = 0;
        android.accounts.Account[] accounts = AccountManager.get(appContext).getAccountsByType(
            ...
            String isPaidAccount)
    {
        ...
        // Prepare account
        Account newAccount = new android.accounts.Account(accountName, AlfrescoAccount.ACCOUNT_TYPE);
        Bundle b = new Bundle();
    ...
    public AlfrescoAccount update(Context context, long accountId, String name, String url, String username,
            ...
            int isPaidAccount)
    {
        android.accounts.Account acc = getAndroidAccount(accountId);
