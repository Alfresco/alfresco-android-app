...
public class PersonSearchLoader extends AbstractPagingLoader<LoaderResult<PagingResult<Person>>>
{
    ...
    public PersonSearchLoader(Context context, AlfrescoSession session, String personIdentifier)
    {
        super(context);
    ...
    public LoaderResult<PagingResult<Person>> loadInBackground()
    {
        ...
        try
        {
            ...
            {
                p = session.getServiceRegistry().getPersonService().search(personIdentifier, listingContext);
            }
        }
        catch (Exception e)
        {
