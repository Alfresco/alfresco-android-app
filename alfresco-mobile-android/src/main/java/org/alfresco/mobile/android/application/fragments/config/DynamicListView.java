 ...
 * and swapping. This layout is in charge of positioning the hover cell in the
 * correct location on the screen in response to user touch events. It uses the
 * position of the hover cell to determine when two cells should be swapped. If
 * two cells should be swapped, all the corresponding data set and layout
 * changes are handled here. If no cell is selected, all the touch events are
 * passed down to the listview and behave normally. If one of the items in the
 * listview experiences a long press event, the contents of its current visible
 * state are captured as a bitmap and its visibility is set to INVISIBLE. A
 * hover cell is then created and added to this layout as an overlaying
 * BitmapDrawable above the listview. Once the hover cell is translated some
 * distance to signify an item swap, a data set change accompanied by animation
 * takes place. When the user releases the hover cell, it animates into its
 * corresponding position in the listview. When the hover cell is either above
 * or below the bounds of the listview, this listview also scrolls on its own so
 * as to reveal additional content.
 */
